...
In the **terra** package, vector data are held in `"SpatVector"` objects. This means that when `read_VECT()` is used, a `"SpatVector"` object is returned, and the same class of object is needed for `write_VECT()` for writing to GRASS.
...
To coerce to and from vector classes defined in the **sp** package, use the classes defined in **sf** as an intermediate step:

```{r, eval=(run && !no_sf)}
Sys.setenv("_SP_EVOLUTION_STATUS_"="2")
no_sp <- inherits(try(require("sp", quietly=TRUE)), "try-error")
```
```{r, eval=(run && !no_sf && !no_sp)}
...
The **stars** package uses GDAL through **sf**:
...
As **rgdal** will retire soon, using **rgdal** in the **raster** package is not a feasible way to coerce a `"SpatRaster"` object to a `"RasterLayer"` object. 
